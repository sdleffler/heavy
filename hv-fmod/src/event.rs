...
impl<'lua> ToLua<'lua> for TimelineMarkerProperties {
    fn to_lua(self, lua: &'lua Lua) -> LuaResult<LuaValue<'lua>> {
...
impl<'lua> ToLua<'lua> for TimelineBeatProperties {
    fn to_lua(self, lua: &'lua Lua) -> LuaResult<LuaValue<'lua>> {
        let table = lua.create_table()?;
        table.set("bar", self.bar)?;
        table.set("beat", self.beat)?;
        table.set("position", self.position)?;
        table.set("tempo", self.tempo)?;
        table.set("time_signature_numerator", self.time_signature_numerator)?;
        table.set(
...
unsafe fn callback_shim(
    ...
    if let Err(err) = result {
        log::error!("error in callback shim: {}", err);
    }
...
impl EventInstance {
    ...
    unsafe fn set_userdata(&self, ud: Arc<BoxedEventCallback>) -> Result<()> {
        if let Some(ud_ptr) = self.get_userdata()? {
            Arc::decrement_strong_count(ud_ptr);
        }
