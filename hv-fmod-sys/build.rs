...
const WINDOWS_PATH: &str =
    r#"C:\Program Files (x86)\FMOD SoundSystem\FMOD Studio API Windows\api\"#;
...
impl FmodPaths {
    pub fn new() -> Self {
        ...
        let core_lib_path = api_root.to_owned()
            + &format!(
                "core{}lib{}x{}",
                path_delimiter, path_delimiter, binary_extension
            );
        let core_lib = "fmod".to_owned() + suffix;
        let studio_lib_path = api_root
            + &format!(
                "studio{}lib{}x{}",
                path_delimiter, path_delimiter, binary_extension
            );
